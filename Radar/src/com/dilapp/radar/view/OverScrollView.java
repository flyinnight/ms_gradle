...
public class OverScrollView extends FrameLayout implements OnTouchListener {
	...
	static final float OVERSHOOT_TENSION = 0;// 0.75f;
	...
	public void initChildPointer() {
		...
		int h = View.MeasureSpec.makeMeasureSpec(0,
				View.MeasureSpec.UNSPECIFIED);
		child.measure(w, h);
		child.setPadding(child.getPaddingLeft(), child.getMeasuredHeight(),
				child.getPaddingRight(), child.getMeasuredHeight());
	}
	 ...
	 * viewport. * @return True if the content fills the viewport, false
	 * otherwise.
	 */
	 ...
	 * * fill the viewport or not. * @param fillViewport True to stretch the
	 * content's height to the * viewport's boundaries, false otherwise.
	 */
	...
	/**
	 * Set whether arrow scrolling will animate its transition. * @param
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if ((action == MotionEvent.ACTION_MOVE) && (mIsBeingDragged)) {
			//Log.i(getClass().getSimpleName(), "intercept true");
			return true;

		}
		switch (action & MotionEvent.ACTION_MASK) {
		case MotionEvent.ACTION_MOVE: { /*
										 * mIsBeingDragged == false, otherwise
										 * the shortcut would have caught it.
										 * Check whether the user has moved far
			...
			//Log.i(getClass().getSimpleName(), "yDiff " + yDiff);
		...
		case MotionEvent.ACTION_UP: /* Release the drag */
			mIsBeingDragged = false;
	...
	private View findFocusableViewInBounds(boolean topFocus, int top, int bottom) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (top < viewBottom && viewTop < bottom) {
				...
				if (focusCandidate == null) { /* No candidate, take this one */
					focusCandidate = view;
	 ...
	 * method will scroll the view to the top or bottom and give the focus to
	 * the topmost/bottommost component in the new visible area. If no component
	 * is a good candidate for focus, this scrollview reclaims the focus. *
	 * </p>
	 * 
	...
	/***
	 * Like {@link View#scrollBy}, but scroll smoothly instead of immediately. * @param
	 * dx the number of pixels to scroll by on the X axis * @param dy the number
	 * of pixels to scroll by on the Y axis
	 */
	...
	public void computeScroll() {
		...
		if (mScroller.computeScrollOffset()) {
			...
			// there // will be a window where getScrollX()/Y is different from
			// what the // app
			// thinks it is. //
			int oldX = getScrollX();
			int oldY = getScrollY();
	 ...
	 * first screen size chunk of it) on screen. * @param rect The rectangle. * @param
	 * immediate True to scroll immediately without animation * @return true if
	 ...
	 * least the first screen size chunk of it). * @param rect The rect. * @return
	 * The scroll delta.
	 */
	...
	protected boolean onRequestFocusInDescendants(int direction,
		...
		// convert from forward
		// backward notation to up / down / left / right // (ugh).
	...
	protected void onScrollChanged(int leftOfVisibleView, int topOfVisibleView,
		...
		if (onScrollChangedListener != null) {
			onScrollChangedListener
					.onScrollChanged(leftOfVisibleView, topOfVisibleView,
							oldLeftOfVisibleView, oldTopOfVisibleView);
			if(topOfVisibleView == child.getHeight() - child.getPaddingTop() - getHeight()) {
				onScrollChangedListener.scrollBottom();
			}
		}
		if (isInFlingMode) {
			// android.util.Log.i("III", "scroll over");
	 ...
	 *            that the finger/cursor is moving down the screen, which
	 *
	...
	public boolean isBottom() {
		return getScrollY() > child.getBottom() - child.getPaddingBottom()
				- getHeight();
