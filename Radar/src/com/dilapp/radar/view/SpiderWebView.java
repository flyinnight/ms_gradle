...
public class SpiderWebView extends View {
    ...
    private int mStartOffset = 50;// 起始值
    ...
    private int command = DRAW_SCENE;
    ...
    void realy() {
        l("ready go! isSetValue " + isSetValue);
        invalidate();
        if (isSetValue) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (command) {
            ...
            case DRAW_VALUE:
                drawScene(canvas, true, true);
                for (int i = 0; i < rulerses.size(); i++) {
                    Paint paint = getIndexElseLast(i, rulersPaints);
                    drawValue(canvas, rulerses.get(i), paint);
                }
                drawSceneWeb(canvas, false);
                drawResultText(canvas, mTip, mTipPaint, mVal, mValPaint, mEval, mEvalPaint);
                break;
    ...
    private void drawScene(Canvas canvas, boolean drawText, boolean drawWeb) {
        ...
        // testDraw(canvas);
        if (mTempRules == null) {
            mTempRules = new float[6];
            int[] temp = new int[getTypes().length];
            for (int i = 0; i < temp.length; i++) temp[i] = getMax();
            values2rulers(temp, getMin(), getMax(), mTempRules);
        }
        l("Max " + getMax());
        drawValue(canvas, mTempRules, pCircleStroke);
    ...
    private float progressRulers(int progress, float ruler) {
        ...
        // }
        // 少了下面这个 f ,动画效果就出不来了
        // float c = mRulerLength / 2f;
        if (ruler <= 0) { // 如果为0的话，会出现图表不显示的问题
            ruler = 0.01f;
        }
        float offset = (float) mStartOffset / mMax;
        return (mRulerLength * offset + ruler * offset) * (progress / 100f);
    }
